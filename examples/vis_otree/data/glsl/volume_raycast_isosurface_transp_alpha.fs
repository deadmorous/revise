...
/* raycast isosurface, transparent, with value alpha */
...
void main(void)
{
    ...
    bool good_prev = has_alpha_texture? texture3D( volume_alpha_texunit, ray_pos ).r == 1: true;
    ...
    do
    {
        ...
        if ( good_cur && good_prev && sign_cur != sign_prev )
        {
            sign_prev = sign_cur;
            vec3 iso_pos = ray_pos - ray_step * 0.5;
            vec4 rgba = computeFragColor( iso_pos );
            float alpha = rgba.a * ALPHA;
            gl_FragColor.rgb += rgba.rgb * transmittance;
            transmittance *= ( 1.0 - alpha );
            gl_FragColor.a = ( 1.0 - transmittance );
            isosurface_found = true;
            if ( gl_FragColor.a > 0.996 )
                break;
        }
        else {
            sign_prev = sign_cur;
