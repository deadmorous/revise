...
__device__ inline std::uint32_t renderIsosurface(const RenderFuncInput& in, const RenderKernelParam& p)
{
    auto& field = p.primaryField;
    auto v1 = in.fieldValue(field.tex);
    increase(in.pos, in.step);

    auto v01 = (p.presentation.threshold - field.minValue) / (field.maxValue - field.minValue);
