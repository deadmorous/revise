...
__device__ inline std::uint32_t renderValueOnIsosurfaces(const RenderFuncInput& in, const RenderKernelParam& p)
{
    ...
    for (int i=1; i<in.sampleCount; ++i, increase(in.pos, in.step)) {
        ...
        while (ilevel >= 0 && ilevel < levelCount) {
            ...
            if ((v1-level)*(v2-level) < 0) {
                //auto v01 = (level - pfield.minValue) / (pfield.maxValue - pfield.minValue);
...
template <>
struct RenderFuncCaller<s3vs::FieldMode::ValueOnIsosurfaces>
{
    __device__ static std::uint32_t call(const RenderFuncInput& in, const RenderKernelParam& p) {
        return renderValueOnIsosurfaces(in, p);
    }
};
